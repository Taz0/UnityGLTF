...
namespace UnityGLTF
{
	internal static class HumanoidSetup
	{
		...
		internal static Avatar AddAvatarToGameObject(GameObject gameObject)
		{
			...
			UnityEngine.Debug.LogError(gameObject.name + ", Meshes = " + skinnedMeshRenderers.Length);

			if (skinnedMeshRenderers.Length > 0)
			{
				UnityEngine.Debug.LogError("Mesh = " + skinnedMeshRenderers[0].name);
			}

			foreach (SkinnedMeshRenderer rend in skinnedMeshRenderers)
			{
				UnityEngine.Debug.LogError("Mesh bones = " + rend.bones.Length);
